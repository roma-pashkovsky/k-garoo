<script lang="ts">
	import { CloseButton, Navbar, NavBrand, NavHamburger, NavLi, NavUl } from 'flowbite-svelte';
...
<Navbar
	navClass="bg-white px-2 sm:px-4 py-2.5 w-full z-20 top-0 left-0 border-b !border-gray-200 dark:!border-gray-600 shadow-sm"
...
>
	...
	<NavUl
		...
		ulClass="relative flex flex-col py-2 px-4 pr-12 md:pr-4 mt-4 md:flex-row md:flex-1 md:justify-end md:space-x-8 md:mt-0 md:text-sm md:font-medium"
	>
		<div class="absolute right-2 top-2 md:hidden">
			<CloseButton on:click={onToggle} class="mb-4 dark:text-white" />
		</div>
