<script lang="ts">
	...
	function onListBodyDblClick(): void {
		saveCurrentList();
		if (itemToEditId) {
			goto(`/home/lists/${list?.id}/edit?editItemId=${itemToEditId}`);
		} else {
			goto(`/home/lists/${list?.id}/edit`);
		}
	}

	function onListItemLongClick(itemId: string): void {
		itemToEditId = itemId;
	}
</script>
						...
						<ul>
							{#each categoryItem.items as item}
								<li
									...
									on:press|stopPropagation={() => onListItemLongClick(item.id)}
									use:swipe={{
										timeframe: 300,
										minSwipeDistance: 80,
